...
const PERSON = {
  ...
  layout:
    ...
    'Row 4 column 0 is the idle fidget. ' +
    'Anchor is bottom-centre of each frame - the character stands on that point.',
  states: ['idle', 'walk', 'talk', 'surprised', 'annoyed', 'happy', 'frightened', 'worried', 'smug', 'fidget'],
};
...
const POSES = [
  ...
  // Row 3 is the expressions row, and the content already asks for it -
  // ['anim', 'jack', 'frightened'] fires in the cabin gag. Every cell used to
  // be a copy of the standing pose, so those beats played silently. Three
  // generations cover all six cells: each reads as two neighbouring moods.
  {
    key: 'front_alarm',
    cells: [[3, 0], [3, 3]],
    brief: 'Facing the camera, alarmed: eyes wide, shoulders up, both hands raised a little in front of the chest, leaning back very slightly.',
  },
  {
    key: 'front_annoyed',
    cells: [[3, 1], [3, 4]],
    brief: 'Facing the camera, thoroughly unimpressed: head tilted, eyebrows down, arms folded, weight on one hip.',
  },
  {
    key: 'front_pleased',
    cells: [[3, 2], [3, 5]],
    brief: 'Facing the camera, quietly delighted with themselves: chin up, small smile, hands on hips or one thumb hooked in a pocket.',
  },
  // An idle fidget, played now and then when a character has been standing
  // still. This is where each of them gets their own bit of business, so the
  // brief is overridden per character below.
  {
    key: 'front_fidget',
    cells: [[4, 0]],
    brief: 'Facing the camera, mid-fidget: shifting weight onto the other foot and adjusting their own clothing, entirely unselfconsciously.',
  },
] as const;

/**
 * Per-character idle business.
 *
 * A fidget is where a character shows what they are like when nothing is
 * happening, so a generic one is a wasted frame. These are the British sitcom
 * register - people dealing with their own clothes, not posing - and they are
 * comic discomfort rather than anything suggestive: a waistband, a bra strap,
 * a jacket that has never fitted.
 */
const FIDGETS: Record<string, string> = {
  'char.jack': 'Facing the camera, hands jammed in his jacket pockets, shoulders hunched, glancing off to one side as though he has just noticed something he would rather not have.',
  'char.maggie': 'Facing the camera, tucking her hair behind one ear and pulling her denim jacket straight, weight shifted onto one hip.',
  'char.arthur': 'Facing the camera, hitching his slacks up by the waistband with both hands and giving his cardigan a tug, entirely unselfconsciously.',
  'char.kevin': 'Facing the camera, hammering an imaginary joystick with both hands, tongue between his teeth.',
  'char.brenda': 'Facing the camera, arms folded, straightening her tabard with a sharp tug and looking at something over your shoulder with deep suspicion.',
  'char.valerie': 'Facing the camera, examining her nails at arm\'s length, one shoulder dropped, thoroughly bored.',
  'char.derek': 'Facing the camera, scratching the back of his head with one hand while the other rummages in the pocket of his shop coat.',
  'char.graham': 'Facing the camera, tugging his too-tight polo shirt down over his stomach and smoothing his moustache with a thumb.',
  'char.guest': 'Facing the camera, gripping the knot of his towel with one hand and standing on his dignity.',
  'char.cabin': 'Facing the camera, hitching her towel up and re-tucking it with a look that dares anyone to comment.',
};
...
for (const [id, def] of Object.entries(CHARACTERS)) {
  ...
  for (const pose of POSES) {
    assets.push({
      ...
      description:
        `${CHARACTER_BRIEFS[id] ?? def.name} ` +
        `POSE: ${(pose.key === 'front_fidget' && FIDGETS[def.sprite]) || pose.brief} ` +
